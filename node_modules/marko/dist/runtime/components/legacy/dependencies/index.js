...
function getRootDeps(template, context) {
    if (production && template.au_) {
        return template.au_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template.au_ = Object.keys(template.av_).map(key => template.av_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target.av_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag.av_) Object.assign(deps, tag.av_);
